...
uint8_t TapeModulator::perm_[512];  // TODO Look back at commit note in DaisySeedProject, may be able to reduce 512 to 256
...
float TapeModulator::GetTapeSpeedLeft() const { return speed_left; }

float TapeModulator::GetTapeSpeedRight() const { return speed_right; }

void TapeModulator::ProcessTapeSpeed(float wow_rate, float flutter_rate, float wow_depth, float flutter_depth, float right_rate_ratio) {
    ...
    // RIGHT ( TODO NOTE - ONLY RECALCULATING FOR WOW for STEREO, USING SAME FLUTTER AS LEFT SIDE (Less processing)
    // Slow WOW component (FBM, smooth)
    float wow_right = Fbm1D(t_wow_right_, octaves_wow_, 2.0f, 0.5f);

    // Tiny FLUTTER component (faster, low amplitude)
    //float flutter = Fbm1D(t_flutter_right_, octaves_flutter_, 2.0f, 0.5f);

    t_wow_right_ += wow_rate * right_rate_ratio / sample_rate_; 
    //t_flutter_right_ += flutter_rate * right_rate_ratio / sample_rate_;

    if(t_wow_right_ >= 256.0f) {
        t_wow_right_ -= 256.0f;
    }
    //if(t_flutter_right_ >= 256.0f) {
    //    t_flutter_right -= 256.0f;
    //}

    // Combine, scaled by depth
    // Assume potentiometer controls the main depth for wow; flutter is small fraction
    speed_left = wow_depth * wow + (flutter_depth * 0.2f) * flutter;
    speed_right = wow_depth * wow_right + (flutter_depth * 0.2f) * flutter;
